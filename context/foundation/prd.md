...
Generatory sklepów do D&D już istnieją, ale wszystkie są jednorazowe — odświeżenie
strony kasuje kupca. Tymczasem sklep w kampanii nie jest zdarzeniem, tylko obiektem,
który żyje miesiącami: gracze wracają do tego samego kowala, pytają o ten sam
przedmiot, wydają zapas, który ktoś musi pamiętać. Trwałość kupca między sesjami
jest tą jedną rzeczą, której status quo nie daje.
...
- FR-009: Ostatni wygenerowany kupiec trwa automatycznie między wizytami MG w aplikacji; jawny zapis oznacza go jako trwałego. Priority: must-have
  > Socrates: Kontrargument przyjęty — "MG w środku sesji nie klika zapisu, tylko
...
> interfejsu), wyszukiwanie, automatyczne utrwalanie i potwierdzenie przed rerollem —
> przy jednoczesnym utrzymaniu edytowalnej tabeli jako must-have. Zakres rośnie w tę
> samą stronę, z której zszedł w Fazie 3. Patrz `## Open Questions` #1.
...
> tę lukę; user wybrał rozkład rzadkości jako prostszy do zbudowania. Patrz
> `## Open Questions` #3.
...
pozostają na urządzeniu MG i nigdy go nie opuszczają.
...
> stała (dostęp z wielu urządzeń), pozostaje nierozwiązana — patrz
> `## Open Questions` #4.

## Non-Goals

Przeniesione wprost z notatek pomysłu:
...
- Gwarancja działania bez internetu — user jej nie wykluczył, ale też nie podniósł do
  rangi NFR w Fazie 5. Aplikacja może działać bez internetu jako skutek uboczny modelu
