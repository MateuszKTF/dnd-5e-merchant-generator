...
active_version: 604061a7-04c3-49a0-b823-2bd15435c0e0
...
- [ ] **Evaluate `output: "static"`.** Every PRD functional requirement is satisfied by a static
      ...
      for nothing. **No longer blocked** — the middleware and API routes it depended on were
      removed on 2026-09-10 (see below). This is now the highest-value open decision: it takes
      the 10 ms CPU ceiling off the path of the PRD's "under 5 seconds" criterion entirely,
      because generation belongs in the React island anyway.
- [x] **Strip the starter auth surface.** ✅ Done 2026-09-10 (commit `3c21436`). Deleted
      `src/middleware.ts`, `src/pages/auth/*`, `src/pages/api/auth/*`,
      `src/pages/dashboard.astro`, `src/components/auth/*`, `src/components/Topbar.astro`,
      `src/lib/supabase.ts`, `src/lib/config-status.ts`; dropped the missing-config banner from
      the layout and the `App.Locals` augmentation. Measured effect: bundle
      **1910 KiB → 1049 KiB** (gzip 391 → 227), build **26 s → 12 s**, the public red Supabase
      banner gone, `/auth/*` and `/api/auth/*` now 404. Supabase packages stay in
      `package.json` as the v2 hook but nothing imports them, so they cost nothing.
...
- [ ] Custom domain (human gate). Cloudflare Access on preview URLs is **no longer needed** on
      licensing grounds: **PRD Open Question #2 is resolved** — the item catalog is SRD 5.1 under
      CC-BY-4.0, generated by `scripts/build-item-catalog.mjs` into `src/data/`, attribution in
      `src/data/ATTRIBUTION.md`. Non-SRD rulebook content (and fan compendia that mirror it,
      5e.tools included) must not be added to this public repo.
- [ ] Supabase-on-workerd spike before committing v2 cloud sync (PRD Open Question #4).
      **Note:** Supabase has no job in v1 at all — the catalog lives in the repo and the auth
      layer is gone, so nothing reads `SUPABASE_URL`/`SUPABASE_KEY`. The `astro:env` schema
      entries remain as the v2 hook. Secrets set in the Cloudflare dashboard on 2026-09-10 never
      reached the Worker (`wrangler secret list` → `[]`); irrelevant for v1, re-do with
      `npx wrangler secret put` if v2 sync happens.
